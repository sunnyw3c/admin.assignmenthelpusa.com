    ...
    <form action="{{ route('writers.update', $writer['id']) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        ...
        <div>
            ...
            <textarea name="bio" rows="3" placeholder="Short description about the writer..."
                class="w-full border border-zinc-200 dark:border-zinc-800 rounded-xl px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 bg-zinc-50 dark:bg-zinc-800/50 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:bg-white dark:focus:bg-zinc-900 focus:outline-none focus:ring-2 focus:ring-accent/40 transition resize-none">{{ old('bio', $writer['bio'] ?? '') }}</textarea>
        </div>
